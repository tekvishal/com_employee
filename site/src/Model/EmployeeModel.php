...
class EmployeeModel extends ItemModel
{
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_employee')) && (!$user->authorise('core.edit', 'com_employee')))
		{
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_EMPLOYEE_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_EMPLOYEE_ITEM_NOT_LOADED'), 404);
			}
		}
